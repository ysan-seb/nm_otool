...
/*   Updated: 2018/11/03 18:59:57 by ysan-seb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			x86_64_not_found(t_stat s, void *ptr)
{
	...
	while (nfat_arch--)
	{
		if (checkoff(s, ptr, swap_or_32(header->magic, arch->offset)))
			return (ERR);
		arch_type(&s, header, arch);
		if (nm(s, ptr + swap_or_32(header->magic, arch->offset)) == ERR)
			return (ERR);
...
int					handle_fat_32(t_stat s, void *ptr)
{
	...
	while (nfat_arch--)
	{
		...
		if (swap_or_32(header->magic, arch->cputype) == CPU_TYPE_X86_64)
			return (nm(s, s.ptr + swap_or_32(header->magic, arch->offset)));
		arch = get_fat_arch_32(s, arch, sizeof(struct fat_arch));
		if (!arch)
			return (ERR);
	}
